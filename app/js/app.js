...
function atualizarTabelas() {
    ...
    const tbodyLivros = document.getElementById("tabelaLivros");
    tbodyLivros.innerHTML = "";
    const selLivro = document.getElementById("selLivro");
    selLivro.innerHTML = "";
    ...
    const tbodyEmp = document.getElementById("tabelaEmprestimos");
    tbodyEmp.innerHTML = "";
...
function cadastrarLivro() {
    ...
    const qtd = parseInt(document.getElementById("qtdLivro").value);
    if (titulo && isbn) {
...
function cadastrarUsuario() {
    ...
    const extra = document.getElementById("extraUsuario").value;
    if (nome && email) {
        let user;
        if (tipo === 'aluno')
            user = new Aluno(nome, email, "123", "aluno", extra, "Curso Padrão");
        else if (tipo === 'professor')
            user = new Professor(nome, email, "123", "professor", extra);
        else
            user = new Funcionario(nome, email, "123", "funcionario", extra);
...
function realizarEmprestimo() {
    const email = document.getElementById("selUsuario").value;
    const isbn = document.getElementById("selLivro").value;
...
function realizarDevolucao(isbn, email) {
    const user = biblioteca.buscarUsuarioPorEmail(email);
    const livro = biblioteca.buscarLivroPorIsbn(isbn);
    if (user && livro) {
